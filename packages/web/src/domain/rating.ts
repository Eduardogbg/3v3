...
const teamAverage = (team: schemas.Player[]) => (
    team.reduce((acc, curr) => acc + curr.mmr, 0) / team.length
)
...
export function calculateRatings(
    ...
    type GameTeams = {
        winningTeam: schemas.Player[]
        losingTeam: schemas.Player[]
    }
    ...
    for (const { winningTeam, losingTeam } of gameTeams) {
        ...
        for (const player of winningTeam) {
            ratings[player.id] += eloDelta(player.mmr, losingTeamRating)
        }

        for (const player of losingTeam) {
            ratings[player.id] -= eloDelta(player.mmr, winningTeamRating);
        }
    }
